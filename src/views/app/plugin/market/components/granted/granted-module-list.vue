...
/** 授权版模块列表：卡片/表格视图 + 分页 + 操作弹窗（无删除功能） */
import { computed, nextTick, ref } from 'vue';

import { ElButton, ElIcon, ElMessage, ElPagination, ElTag } from 'element-plus';
...
import GrantedModuleInstallDialog from './granted-module-install-dialog.vue';
import GrantedModuleUninstallDialog from './granted-module-uninstall-dialog.vue';
import GrantedUpgradeLog from './granted-upgrade-log.vue';

defineOptions({ name: 'GrantedModuleList' });
...
const tableGridOptions = computed<any>(() => ({
  ...
  columns: [
    {
      type: 'seq',
      title: $t('app.plugin.market.column_seq'),
      width: 60,
      align: 'center',
    },
    {
    ...
    {
      field: 'version',
      title: $t('app.plugin.market.column_version'),
      width: 100,
      align: 'center',
    },
    {
...
// ── 操作 ────────────────────────────────────
const installModule = async (mod: any) => {
...
const updateModule = async (mod: any) => {
  try {
    await requestClient.post(`/tenant/plugin/${mod.code || mod.key}/update-db`);
    ElMessage.success(
      $t('app.plugin.market.update.success', { name: mod.name }),
    );
    emit('update', mod);
  } catch (error: any) {
...
<template>
  <div class="module-list-container">
        ...
        >
          ...
          <div class="card-footer">
            <div class="action-row">
              ...
              <!-- 可更新 → 更新数据库 -->
              <ElButton
                v-else-if="mod.has_update"
                type="warning"
                size="small"
                class="flex-1"
                @click.stop="updateModule(mod)"
              >
    ...
    >
      ...
      <template #action="{ row }">
        <div class="table-actions">
          ...
          <!-- 可更新 → 更新数据库 -->
          <ElButton
            v-else-if="row.has_update"
            type="warning"
            size="small"
            @click.stop="updateModule(row)"
          >
            {{ $t('app.plugin.market.update_db') }}
          </ElButton>
          <!-- 未安装 → 安装菜单 -->
    ...
    <!-- 安装弹窗 -->
    <GrantedModuleInstallDialog
      ref="installDialogRef"
      :plugin="selectedModuleForInstall"
      @close="selectedModuleForInstall = null"
      @refresh="emit('install', selectedModuleForInstall)"
    />
    <!-- 卸载弹窗 -->
    <GrantedModuleUninstallDialog
    ...
    <GrantedUpgradeLog
      ref="upgradeLogRef"
      @close="selectedModuleForUpgrade = null"
    />
  </div>
</template>
