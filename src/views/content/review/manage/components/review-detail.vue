...
import type {
  ReviewDetail,
  ReviewRow,
} from '#/api/content/review/manage/types';
...
async function loadDetail(id: number) {
  loading.value = true;
  try {
    detail.value = await reviewManageService.detail(id);
  } finally {
    loading.value = false;
  }
}
...
const [Dialog, dialogApi] = useDialog({
  title: $t('content.review.manage.detail.title'),
  width: '720px',
  dialogType: 'drawer',
  footer: false,
  destroyOnClose: true,
  ...
  <Dialog>
    <div v-loading="loading" class="p-2">
      <ElAlert
        ...
        :title="$t('content.review.manage.detail.archived_tip')"
        type="info"
        :closable="false"
        class="mb-3"
      />
      ...
      <ElTabs v-model="activeTab">
        <!-- 审核信息 -->
        <ElTabPane
          :label="$t('content.review.manage.detail.tabs.info')"
          name="info"
        >
          <ElDescriptions :column="2" border size="small">
            <ElDescriptionsItem
              :label="$t('content.review.manage.table.columns.id')"
            >
            ...
            <ElDescriptionsItem
              :label="$t('content.review.manage.detail.created_by')"
            >
        ...
        <!-- 表单数据 -->
        <ElTabPane
          :label="$t('content.review.manage.detail.tabs.form')"
          name="form"
        >
          <ElEmpty
            v-if="!detail?.form || Object.keys(detail.form).length === 0"
            :description="$t('content.review.manage.detail.no_form')"
          />
        ...
        <!-- 状态 -->
        <ElTabPane
          :label="$t('content.review.manage.detail.tabs.status')"
          name="status"
        >
          <ElDescriptions :column="2" border size="small">
            <ElDescriptionsItem
              :label="$t('content.review.manage.table.columns.status')"
            >
            ...
            <ElDescriptionsItem
              :label="$t('content.review.manage.detail.flow_type')"
            >
              {{ detail?.status?.flow_type }}
            </ElDescriptionsItem>
            <ElDescriptionsItem
              :label="$t('content.review.manage.table.columns.reviewer')"
            >
              {{ detail?.status?.reviewer_id }}
            </ElDescriptionsItem>
            <ElDescriptionsItem
              :label="$t('content.review.manage.table.columns.reviewed_at')"
            >
              {{ formatDateTime(detail?.status?.reviewed_at) }}
            </ElDescriptionsItem>
            <ElDescriptionsItem
              v-if="detail?.status?.reason"
              :label="$t('content.review.manage.detail.reason')"
            >
              {{ detail?.status?.reason }}
            </ElDescriptionsItem>
            <ElDescriptionsItem
              v-if="detail?.status?.cancel_reason"
              :label="$t('content.review.manage.detail.cancel_reason')"
            >
        ...
        <!-- 事件记录 -->
        <ElTabPane
          :label="$t('content.review.manage.detail.tabs.events')"
          name="events"
        >
          <ElEmpty
            v-if="!detail?.events || detail.events.length === 0"
            :description="$t('content.review.manage.detail.no_events')"
          />
          <ElTimeline v-else>
            <ElTimelineItem
