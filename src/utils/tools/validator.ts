...
export function validateTelPhone(value: string): boolean {
  ...
  return telRegex.test(value.trim().replaceAll(/\s+/g, ''));
}
...
export function validateChineseIDCard(value: string): boolean {
  ...
  for (const [index, weight] of weights.entries()) {
    sum += Number.parseInt(trimmedValue.charAt(index), 10) * weight;
  }

  const checkCode = checkCodes[sum % 11];
  return (
    checkCode !== undefined &&
    trimmedValue.charAt(17).toUpperCase() === checkCode
  );
}
...
export function validateBankCard(value: string): boolean {
  ...
  for (let i = trimmedValue.length - 1; i >= 0; i--) {
    let digit = Number.parseInt(trimmedValue.charAt(i), 10);
