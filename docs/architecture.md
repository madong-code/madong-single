...
本项目是基于 Vue Vben Admin `v5.7.0` 重构的 Element Plus 单应用版本。根目录 `src` 是唯一运行时应用，不再保留 `apps/*`、`playground/*` 或运行时 `packages/*`。
...
| `locales`     | 框架国际化能力                          |
| `plugins`     | ECharts、Tiptap、VXE Table 等第三方集成 |
| `preferences` | 主题及偏好设置                          |
| `request`     | 请求客户端和拦截器                      |
...
TypeScript、Vite 和 Vitest 中的 `#` 别名均指向根 `src`：
...
运行时源码不使用 `@vben/*`、`@vben-core/*` 或 `@/*`。工程工具包可以继续使用 `@vben/*` workspace 包名。
