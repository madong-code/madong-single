...
5. 新工程配置放入 `build` 或 `tooling`，避免增加根目录杂项。
6. `src/core` 保持 11 个能力域，不增加额外一级抽象。
...
- TypeScript、Vite 和 Vitest 的 `#` 别名是否一致。
- 根 `package.json` 是否直接声明新增运行时依赖。
